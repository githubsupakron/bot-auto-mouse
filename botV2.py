...
last_time = None
running = False
round_count = 0  # นับจำนวนรอบที่ทำงานเสร็จ
...
def on_click(x, y, button, pressed):
    ...
    if pressed and recording:
        ...
        if len(click_steps) == 0:
            ...
            wait_time = round(now - last_time, 2)
        last_time = now
...
def start_recording():
    global recording, last_time, click_steps
    clear_all_steps()
    recording = True
    last_time = None
...
def stop_recording():
    global recording
    recording = False
    status_label.config(text="⏹ หยุดบันทึกแล้ว")

def run_bot():
    global running, round_count
    if not running and click_steps:
        running = True
        round_count = 0  # เริ่มต้นใหม่ทุกครั้งที่รัน
...
# GUI
root = tk.Tk()
root.title("🍄 Bot Recorder V2.6 – เก็บเห็ดอัตโนมัติ")
